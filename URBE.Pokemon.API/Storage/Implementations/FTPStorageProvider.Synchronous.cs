...
public partial class FTPStorageProvider
{
    ...
    private FtpClient CreateAndConnect()
    {
        lock (syncClientLock)
        {
            ...
            {
                syncClient = new FtpClient(Host, Username, Password, Port, ValidateAnyCertificate ? NoCertificateConfig : DefaultConfig);
    ...
    public DisposalManager<Stream> GetReadStream(string path)
    {
        ...
        return new(stream, true, () => { ThrowIfNotSuccess(ftp); ftp.Dispose(); return ValueTask.CompletedTask; });
    }
