...
public partial class FTPStorageProvider
{
    ...
    private async Task<AsyncFtpClient> CreateAndConnectAsync(CancellationToken ct)
    {
        lock (asyncClientLock)
        {
            ...
            {
                asyncClient = new AsyncFtpClient(Host, Username, Password, Port, ValidateAnyCertificate ? NoCertificateConfig : DefaultConfig);
    ...
    public async ValueTask<DisposalManager<Stream>> GetReadStreamAsync(string path, CancellationToken ct = default)
    {
        ...
        return new(stream, true, async () => { await ThrowIfNotSuccessAsync(ftp, ct); ftp.Dispose(); });
    }

    public async ValueTask<DisposalManager<Stream>> GetWriteStreamAsync(string path, FileMode mode, CancellationToken ct = default)
    {
        var ftp = await CreateAndConnectAsync(ct);
